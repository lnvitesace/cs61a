...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for bee in list(bees):
            Insect.reduce_armor(bees[bees.index(bee)], amount)
        if self.armor <= amount:
            for bee in list(bees):
                Insect.reduce_armor(bees[bees.index(bee)], self.damage)
...
# The WallAnt class
class WallAnt(Ant):
    """Wall Ant is an ant that does nothing each turn. 
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a subclass of ThrowerAnt that is more 
    ...
    def action(self, gamestate):
        ...
        if self.is_true:
            ...
            while curr is not None:
                if curr.ant and not curr.ant.damage_doubled:
                    curr.ant.damage *= 2
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        bees = self.place.bees
        for bee in list(bees):
            bees[bees.index(bee)].reduce_armor(self.damage)
        # END Problem Optional 1
