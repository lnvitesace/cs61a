...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        possibilities.append(("+[" + code_char + "]" + insert_result, insert_cost+1))
        # Delete
        delete_result, delete_cost = helper_align(skeleton_idx+1, code_idx)
        possibilities.append(("-[" + skel_char + "]" + delete_result, delete_cost+1))
...
def filterl(lst, pred):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(acc, elemtent):
        if pred(elemtent):
            return Link(elemtent, acc)
        return acc
    filtered_list = foldl(lst, helper, Link.empty)
    return reverse(filtered_list) #reverse is implemented below
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    def prepen(acc, element):
        return Link(element, acc)
    return foldl(lst, prepen, Link.empty)
